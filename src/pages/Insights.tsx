...
const Insights = () => {
    ...
    const fetchInsights = async () => {
      try {
        ...
        if (data && data.length > 0) {
          // Mapear campos do banco para o formato do frontend se necessário
  ...
  // Usar dados dinâmicos se existirem, senão usar os estáticos (fallback)
  const currentInsights = dynamicInsights.length > 0 ? dynamicInsights : staticInsights;
            ...
            <div>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {regularPosts.map((post) => (
                  <Link key={post.id} to={`/insights/${post.slug}`} className="group flex flex-col h-full bg-card rounded-2xl overflow-hidden border border-border hover:border-secondary/30 hover:shadow-lg transition-all duration-300">
